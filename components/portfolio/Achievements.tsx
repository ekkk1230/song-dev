...
// 1. 프로젝트 비주얼 그래픽 컴포넌트 (rem 단위 완벽 적용)
...
function ProjectRow({ project, idx }: { project: Project; idx: number }) {
    ...
    return (
        <article
            ref={root}
            className="grid items-center gap-[2.5rem] border-t border-border py-[4rem] md:grid-cols-2 md:gap-[4rem] md:py-[6rem]"
        >
            {/* 좌우 지그재그 시각화 영역 */}
            <div
                data-visual
                data-cursor-hover
                className={`group relative aspect-[4/3] overflow-hidden rounded-[1rem] border border-border bg-card transition-transform duration-500 hover:scale-[1.02] ${
                    reversed ? 'md:order-2' : ''
                }`}
            >
                <ProjectVisual variant={project.variant} />
                <span className="pointer-events-none absolute right-[1.25rem] top-[1.25rem] font-heading text-[3.75rem] font-bold text-foreground/10">
                    {project.index}
                </span>
            </div>
            ...
            <div className={reversed ? 'md:order-1' : ''}>
                ...
                <h3 
                    data-reveal
                    className="mt-[1.25rem] font-heading text-[1.875rem] font-semibold leading-tight break-keep text-foreground md:text-[2.25rem]"
                >
                    {project.title}
                </h3>
                ...
                <p
                    data-reveal
                    className="mt-[1rem] max-w-[28rem] text-pretty leading-relaxed break-keep text-muted-foreground"
                >
                    {project.description}
                </p>

                <ul data-reveal className="mt-[1.5rem] space-y-[0.5rem]">
                    {project.highlights.map((h, hIdx) => (
                        <li
                            key={hIdx}
                            className="flex gap-[0.75rem] text-[0.875rem] leading-relaxed break-keep text-foreground/80"
                        >
                            <ArrowUpRight className="mt-[0.125rem] h-[1rem] w-[1rem] shrink-0 text-primary" />
                            <span>{h}</span>
                        </li>
                    ))}
                </ul>

                <ul data-reveal className="mt-[1.75rem] flex flex-wrap gap-[0.5rem]">
                    {project.stack.map((tech) => (
                        <li
                            key={tech}
                            className="rounded-full border border-border px-[0.75rem] py-[0.25rem] font-mono text-[0.75rem] text-muted-foreground"
                        >
                            {tech}
                        </li>
                    ))}
...
// 3. 최종 Achievements 메인 섹션 수출 컴포넌트 (rem 단위 완벽 적용)
